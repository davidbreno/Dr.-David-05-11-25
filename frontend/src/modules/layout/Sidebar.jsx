...
export default function Sidebar() {
  ...
  const MenuItem = ({ to, icon, label, onClick }) => {
    ...
    return (
      <Link
        to={to}
        className={`flex items-center gap-3 mx-6 px-4 py-2.5 rounded-xl transition-all group ${
          active
            ? 'bg-[#7DEDDE] text-[#1a1b26] font-semibold'
            : 'text-gray-400 hover:text-white hover:bg-[#21222D]'
        }`}
      >
        {icon}
        <span className="text-sm font-medium">{label}</span>
